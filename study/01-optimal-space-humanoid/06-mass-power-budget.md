...
| Actuation (38 joints: NdFeB motors + harmonic drives + QDD wrist/hand motors + encoders) | 13.0 | §03, Section 4 | Dominant actuation mass item; 38 joints × ~342 g mean actuator assembly mass (HD-Electric primary at load-bearing joints, QDD at wrists/hands). Atlas Electric heritage anchor. |
...
| Thermal management (torso + battery MLI blankets, resistive survival heater elements, heat pipes, waste-heat radiator panel, joint-region spot MLI) | 3.0 | §05, Section 2; parametric derivation | MLI blankets ~0.8 kg; heater elements ~0.2 kg; heat pipes ~0.8 kg; radiator panel ~1.2 kg. Radiator sized to reject ~300 W at end-of-mission emissivity (ε = 0.70 EOL per §05); at 50°C panel temperature, required radiator area ~0.3 m² at ~10 kg/m² = ~3.0 kg. This is a concept-phase estimate; detailed thermal model is required. |
...
| **Allocated subsystem subtotal** | **58.2** | Sum of above | This is the sum of all individually allocated subsystems. Growth allowance (below) covers unallocated reserve within the design-to. |
| System growth allowance (unallocated reserve at design-to level) | 16.8 | 75 kg design-to target minus allocated subtotal | 16.8 kg = 29% of allocated subtotal. This is consistent with concept-phase MGA practice (AIAA S-120, which recommends 20–30% MGA at system level for TRL 3–5 hardware at concept phase). The large fraction reflects the number of subsystems below TRL 5 in the current design. |
...
**Notes on budget closure:** The budget closes — the sum of estimated subsystem masses (58.2 kg) is meaningfully below the 75 kg design-to target, leaving 16.8 kg of unallocated system growth allowance (29% of allocated subtotal). The power system (16.9 kg, 22% of total) is the largest single allocated subsystem. The structure + actuation block totals 30.0 kg as established in §03. Sensors and compute together total 3.9 kg, consistent with the §04 prediction that these are not the mass driver.

The 29% unallocated growth allowance is deliberate and should not be interpreted as slack to be consumed in early design phases. At this concept level, no subsystem mass estimate has been backed by a detailed parts list; all are parametric from heritage analogs. History shows that the gap between concept-phase parametric estimates and first-article hardware mass is consistently in the 20–30% range — which is why NASA-STD-5001 requires the 30% system-level margin on top of the design-to. The 16.8 kg unallocated reserve within the design-to represents the expectation that detailed design will consume some portion of it before the system-level margin is invoked.
...
The full locomotion + manipulation mode is the peak-draw scenario; it must close within 800 W with margin, which means ≤615 W design-to. Stationary manipulation must close within 500 W with margin (≤385 W design-to). Lunar night survival must close within 150 W with margin (≤115 W design-to at the lower bound of the 70–200 W thermal range).
...
| Thermal — survival heaters (electronics + battery compartments; off during active ops) | 0 | 0 | 85–175 |
...
**Thermal hibernation note:** The lunar night survival power range (114–234 W pre-margin; 148–304 W with margin) reflects the wide uncertainty in the thermal model for the bipedal form factor. At the lower bound (114 W pre-margin), the budget closes within the 150 W goal with margin barely (148 vs. 150 W). At the upper bound (234 W pre-margin), the margin-included total of 304 W substantially exceeds the 150 W goal. This is the single place where the power budget does not close against the stated goal at the upper bound of uncertainty. The root cause is the wide parametric range in §A10 (70–200 W FSP thermal reservation), which was set at TRL 2 without a validated thermal model. The study's recommended response is not to tighten the number by assumption, but to flag it as a priority input for the detailed thermal modeling program (must reach TRL 5 by 2031 per §05 TRL table). If the thermal model resolves to the upper bound, the FSP allocation per humanoid unit during lunar night must be treated as up to 300 W rather than 150 W, and the far-side-base-architect must revise the FSP sizing accordingly. With three humanoids deployed, this worst-case FSP reservation becomes ~900 W continuously through each 14-day lunar night — approximately 9% of a 10 kWe FSP plant. This is significant and must be reconciled before PDR.

---

## 3. Budget Closure Assessment

**Does the budget close?** Yes, with two qualifications. The mass budget closes with 16.8 kg of unallocated system growth allowance within the 75 kg design-to, and the 97.5 kg NTE accommodates an additional 30% system-level margin beyond that. The power budget closes for the full locomotion + manipulation mode (616 W vs. 800 W cap) and the stationary manipulation mode (432 W vs. 500 W cap), and closes at the lower bound of uncertainty for lunar night hibernation (148 W vs. 150 W goal). Budget closure is therefore conditional on the thermal hibernation range resolving to the lower half of the parametric estimate.

**Top two stress points:** First, the battery/power system subsystem (16.9 kg, 22% of allocated mass) is the largest single mass line item and is sensitive to the energy density assumption. If space-qualified cells remain at 150 Wh/kg rather than the 160 Wh/kg design-to value, the battery mass grows by ~0.8 kg — manageable. If operational depth-of-discharge margins require 25% additional capacity reserve (a credible requirement for cold-temperature operation where Li-ion capacity degrades at the sub-zero temperatures prior to full warm-up), the battery mass grows by ~3.1 kg, consuming 18% of the growth allowance. Second, the lunar night thermal power draw is unconstrained by analysis: the 70–200 W range from §A10 represents a 3× uncertainty span, and the upper bound breaks the stated 150 W survival power goal. Until a validated thermal model bounds this range to better than ±30 W, this is the most consequential open item in the power budget.
...
| HD-Electric actuation mass | §A4 | If SEA fallback at 2029 gate: actuation subsystem grows by 10–20 kg. Growth allowance (16.8 kg) partially absorbs this, but the NTE likely breaks at the upper end. Mass budget must be renegotiated. |
...
| Lunar night survival power goal ≤150 W | §A3 note | Upper bound of thermal estimate (304 W with margin) exceeds goal. Far-side-base-architect must provision FSP for worst case until thermal model closes. |
...
\cite{bostondynamics2024atlas} — Atlas Electric spec sheet; 89 kg platform, HD-electric actuation, 85–90% efficiency.
...
\cite{paine2015valkyrieActuator} — Valkyrie SEA actuator per-joint power data; used for locomotion actuation budget derivation.
