...
A bilateral, two-legged anthropomorphic robot of human scale (1.5–1.9 m standing height, 75 kg design-to per this study's commitment (Section 4); heritage mass range 57–89 kg for commercial platforms), with two dexterous arms and human-geometry hands. The canonical form factor: Atlas, Valkyrie, FEDOR, Optimus, Apollo.
...
| **Weighted total** | 1.00 | **3.65** | **3.55** | **3.25** | **2.90** | **2.85** |
...
**The bipedal form wins the tradespace evaluation by a margin of 0.10 over the centaur (3.65 vs. 3.55).** This is a narrow gap, and it is honest. The bipedal wins primarily on C1 (human tool and environment compatibility, weight 0.25) and C7 (training overhead, weight 0.05). It loses on mass (C5, score 4 vs. 4 — tied) and scores average on locomotion (C2, score 3 vs. 4 — centaur wins). The 0.10 gap means the position rests on the C1 weighting, which the study has assigned based on the permanent-base thesis: the base is designed for humans, the robot must work in it. A reviewer who disagrees with that weighting should engage the economics argument below.
...
The Lunokhod counterargument must be engaged directly. Lunokhod 1 and Lunokhod 2 together traversed approximately 50 km of lunar surface (Lunokhod 1: 10.5 km over 11 months; Lunokhod 2: 39.2 km per LRO photogrammetric cartographic revision \cite{lroc_lunokhod2_traverse}; the 39.2 km figure supersedes the original ~37 km estimate from Soviet telemetry) and returned more scientific data per kilogram of delivered hardware than any other robotic lunar mission. The Soviet engineers built a rover optimized for the lunar surface — eight independently-driven wheels, rocker-bogey suspension, pressurized electronics compartment, lid-mounted solar array — and it worked. The lesson drawn by Soviet and post-Soviet designers is that purpose-built beats generalist in resource-constrained environments. FEDOR itself was the Soviet attempt to apply the same philosophy to a humanoid: build a capable teleop avatar, don't try to generalize.
...
### Program Economics: Single-Standard vs. Dual-Standard Architecture

This subsection quantifies the economics argument directionally. The analysis is parametric and provisional; the ConOps section (Question c) will produce the definitive task taxonomy.

**(a) Rough task taxonomy.** Based on analogy to ISS maintenance operations and the expected task set for a permanent lunar far side base, base tasks fall into three categories:

- **Human-geometry-required tasks** (~40–60% of task volume): airlocks, suit donning/doffing, IVA operations inside pressurized habitat, shared tool use from standard EVA tool kits, rack equipment access, emergency crew assist. These tasks require a robot that fits human-scale hatches, reaches human-scale racks, and grasps human-scale tools. The ISS analogy is the strongest available data point: roughly half of Robonaut 2's planned task set was explicitly designed around human-geometry tool compatibility, and ISS maintenance operations skew heavily toward human-geometry interfaces.
- **Adaptable tasks** (~25–35%): outdoor cargo logistics, external equipment transport on prepared paths, science instrument deployment on flat terrain. These could be performed by a non-human-geometry platform with minor adaptation to attachment interfaces.
- **Bipedal-locomotion-specific tasks** (~10–15%): hazardous terrain traversal, ladder and stairwell access, fall-and-recovery procedures in EVA support. Require bipedal geometry specifically.

On this taxonomy, approximately 50–75% of the total task volume requires or benefits from human-geometry form. The residual ~25–35% is the window where a purpose-built alternative could be more efficient.

**(b) Recurring cost delta (parametric).** A two-robot-type program — one bipedal humanoid for human-geometry tasks, one purpose-built platform for logistics and terrain work — would incur recurring overhead costs not present in a single-type program. Order-of-magnitude estimates:

- Operator training: ~200 operator-hours/robot-type/year × 2 types = 400 vs. 200 operator-hours/year. At a loaded crew-hour cost of ~$10–25k/hr for a lunar far side deployment, recurring training overhead is ~$2–4M/year differential.
- Spare parts manifests: a two-type program roughly doubles unique part numbers at the base, increasing logistics complexity and resupply manifest mass. Conservatively, this adds ~$0.5–1M/year in logistics overhead once the base is operational.
- Software maintenance: two autonomous control stacks require two development and maintenance teams. At commercial software team rates, this is approximately $1–3M/year additional.
- Tool and adapter sets: separate fixture sets for non-standard robot geometry add to initial base manifest (~0.5–1M one-time).

Total estimated recurring overhead for a two-type program: approximately **$3–8M/year** over a 20-year permanent base, or **$60–160M total program lifecycle cost** attributable to the dual-standard architecture.

**(c) Crossover sensitivity.** The bipedal form's mass penalty versus a task-optimized alternative is approximately 10–25 kg (the difference between a logistics-optimized non-humanoid platform and the 75 kg bipedal). At current launch cost estimates of ~$5–10k/kg to the lunar surface for Starship HLS-class vehicles, the recurring launch cost of carrying this mass penalty across 20 units deployed over 20 years is approximately $1–5M total. This is **small compared to the estimated $60–160M in dual-standard recurring overhead.** The crossover point — where the mass penalty cost equals the dual-standard overhead cost — is well below the study's assumptions. The economics argument is directionally correct; the bipedal form's mass and locomotion penalties do not come close to the program cost of maintaining two parallel tool and infrastructure standards across a 20-year base lifetime.

**(d) Caveats and gate.** The task taxonomy above is provisional and built on ISS analogy, not a defined task list for the lunar far side base. The economic analysis flows from the taxonomy; if the taxonomy is wrong, the analysis is wrong. **Gate: if the ConOps task taxonomy in Section 03 shows fewer than 40% of tasks requiring human-geometry access, the economic argument must be re-evaluated and the form factor position must be reconsidered.** The 40% threshold is the level at which the recurring cost delta from training, spares, and software maintenance begins to approach the mass-penalty cost. Below that threshold, a dual-type architecture may be economically defensible. The ConOps section owns this validation.
...
\cite{huntress2011soviet} — Lunokhod 1/2 design philosophy and scientific return; Lunokhod 1 traverse (10.5 km over 11 months). The Lunokhod 2 traverse distance of 39.2 km is cited separately to \cite{lroc_lunokhod2_traverse}.
\cite{lroc_lunokhod2_traverse} — LRO LROC NAC photogrammetric cartographic revision establishing Lunokhod 2 traverse at 39.16 km, superseding Soviet telemetry estimate of ~37 km in Huntress & Marov (2011).
