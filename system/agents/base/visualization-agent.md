...
**Chart index:** `cycles/cycle-<NN>/<section>/` (path set by cycle scaffold)
...
5. Update `cycles/cycle-<NN>/<section>/` (path set by cycle scaffold) with an entry for the new chart.
...
2. **mass-budget-breakdown.png** — Pie chart of primary system mass by subsystem from `06-mass-power-budget.md` once it exists.
3. **comms-latency-by-destination.png** — Horizontal bar chart: Earth-Moon (2.6 s RTT), Earth-Mars (8–48 min range), Earth-Jupiter (70–100 min). operational site relay overhead noted. Log or linear scale as appropriate.
...
Footer watermark on every chart: `primary system-Forward Space Study — Auto-generated YYYY-MM-DD`
